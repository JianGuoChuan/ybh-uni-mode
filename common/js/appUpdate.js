...
// 主颜色
let $mainColor = "#2A9FD1";
// 弹窗图标url
const $iconUrl = require('@/static/img/fire.png');

let appUpdate = {
	// 文字换行
	drawtext(text, maxWidth) {
		$mainColor = uni.getStorageSync('themeData')? uni.getStorageSync('themeData').vuex_themeColor.color: "#2A9FD1";
			...
			if (/[\u4e00-\u9fa5]|[\uFE30-\uFFA0]/g.test(textArr[i])) {
				if(letterWidth > 0){
					if(nodeWidth + chineseWidth + letterWidth * otherWidth > maxWidth){
						rowText.push({ type: "text", content: text.substring(previousNode, i) });
						previousNode = i;
				...
				if(/\n/g.test(textArr[i])){
					rowText.push({ type: "break", content: text.substring(previousNode, i) });
					previousNode = i + 1;
					nodeWidth = 0;
					letterWidth = 0;
					...
					if(nodeWidth + letterWidth * otherWidth > maxWidth){
						rowText.push({ type: "text", content: text.substring(previousNode, i + 1 - letterWidth) });
					...
					}else{ nodeWidth += otherWidth; }
				}
			}
		}
		if (previousNode < len) {
			rowText.push({ type: "text", content: text.substring(previousNode, len) });
		}
		return rowText;
	},
	
	// 是否更新弹窗
	updatePopup(data, callback) {
		// 弹窗遮罩层
		...
		// 描述的列表
		const descriptionList = this.drawtext(data.versionInfo, viewContentWidth);
		// 弹窗容器高度
	...
	downloadPopupDrawing(data){
		...
		let elementList = [
			{
				tag: 'rect', //背景色
				color: '#FFFFFF',
				rectStyles:{ radius: "8px" }
			},
			{
				tag: 'font',
				id: 'title',
				text: "升级APP",
				textStyles: { size: '16px', color: "#333", weight: "bold", verticalAlign: "middle", whiteSpace: "normal" },
				position: {
			...
			},
			{
				...
				id: 'content',
				text: contentText,
				textStyles: { size: '14px', color: "#333", verticalAlign: "middle", whiteSpace: "normal" },
				position: {
			...
			elementList = elementList.concat([
				{
					...
					id: 'progressValue',
					text: progressTip,
					textStyles: { size: '14px', color: $mainColor, whiteSpace: "normal" },
					position: {
				...
				},
				{
					tag: 'rect', //绘制进度条背景
					id: 'progressBg',
					rectStyles:{ radius: "4px", borderColor: "#f1f1f1", borderWidth: "1px" },
					position:{
		...
		if (data.buttonNum == 2) {
			...
			elementList = elementList.concat([
				{
					tag: 'rect', //绘制底边按钮
					rectStyles:{ radius: "3px", borderColor: "#f1f1f1", borderWidth: "1px" },
					position:{
						bottom: viewContentPadding + 'px',
						left: viewContentPadding + "px",
						width: (viewContentWidth - viewContentPadding) / 2 + "px",
				...
				{
					tag: 'rect', //绘制底边按钮
					rectStyles:{ radius: "3px", color: $mainColor },
					position:{
						bottom: viewContentPadding + 'px',
						left: ((viewContentWidth - viewContentPadding) / 2 + viewContentPadding * 2) + "px",
						width: (viewContentWidth - viewContentPadding) / 2 + "px",
				...
				},
				{
					...
					id: 'cancelText',
					text: "取消下载",
					textStyles: { size: '14px', color: "#666", lineSpacing: "0%", whiteSpace: "normal" },
					position: {
						bottom: viewContentPadding + 'px',
						left: viewContentPadding + "px",
						width: (viewContentWidth - viewContentPadding) / 2 + "px",
				...
				},
				{
					...
					id: 'confirmText',
					text: "后台下载",
					textStyles: { size: '14px', color: "#FFF", lineSpacing: "0%", whiteSpace: "normal" },
					position: {
						bottom: viewContentPadding + 'px',
						left: ((viewContentWidth - viewContentPadding) / 2 + viewContentPadding * 2) + "px",
						width: (viewContentWidth - viewContentPadding) / 2 + "px",
		...
		if (data.buttonNum == 1) {
			...
			elementList = elementList.concat([
				{
					tag: 'rect', //绘制底边按钮
					rectStyles:{ radius: "6px", color: $mainColor },
					position:{
						bottom: viewContentPadding + 'px',
						left: viewContentPadding + "px",
						width: viewContentWidth + "px",
				...
				},
				{
					...
					id: 'confirmText',
					text: "关闭",
					textStyles: { size: '14px', color: "#FFF", lineSpacing: "0%" },
					position: {
						bottom: viewContentPadding + 'px',
						left: viewContentPadding + "px",
						width: viewContentWidth + "px",
	...
	downloadPopup(data) {
		...
		let popupViewData = this.downloadPopupDrawing(data);
		// 弹窗内容
		let popupView = new plus.nativeObj.View("popupView", { //创建底部图标菜单
			tag: "rect",
			...
			change: function(res) {
				...
				if(res.progressValue){
					...
					progressElement.push({
						tag: 'rect', //绘制进度条背景
						id: 'progressValueBg',
						rectStyles:{ radius: "4px", color: $mainColor },
						position:{
				...
				if(res.contentText){
					...
					progressElement.push({
						...
						textStyles: { size: '16px', color: "#333", whiteSpace: "normal" },
						position: {
				...
				if(res.buttonNum >= 0 && buttonNum != res.buttonNum){
					...
					popupView.reset();
					popupViewData = this.downloadPopupDrawing(Object.assign({
