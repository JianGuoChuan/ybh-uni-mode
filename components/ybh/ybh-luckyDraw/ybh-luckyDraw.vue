	...
	<view class="almost-lottery">
		...
		<view class="almost-lottery__wrap" :style="{ width: lotterySize + 'rpx', height: lotterySize + 'rpx' }">
			...
			<template v-if="lotteryImg">
				<image class="almost-lottery__bg" mode="widthFix" :src="lotteryBg" :style="{ width: lotteryPxSize + 'px', height: lotteryPxSize + 'px' }" ></image>
				<image class="almost-lottery__canvas-img" mode="widthFix" :src="lotteryImg" :style="{ width: canvasImgPxSize + 'px', height: canvasImgPxSize  + 'px', left: canvasImgToLeftPx + 'px', top: canvasImgToLeftPx + 'px', transform: `rotate(${canvasAngle + targetAngle}deg)`, transitionDuration: `${transitionDuration}s` }" ></image>
				<image class="almost-lottery__action-bg" mode="widthFix" :src="actionBg" :style="{ width: actionPxSize + 'px', height: actionPxSize + 'px', left: actionBgToLeftPx + 'px', top: actionBgToLeftPx + 'px', transform: `rotate(${actionAngle + targetActionAngle}deg)`, transitionDuration: `${transitionDuration}s` }" @click="handleActionStart" ></image>
			</template>
		</view>	
		...
		<!-- #ifdef MP-ALIPAY -->
		<canvas :class="className" :id="canvasId" :width="higtCanvasSize" :height="higtCanvasSize" 
			:style="{ width: higtCanvasSize + 'px', height: higtCanvasSize + 'px' }" />
		<!-- #endif -->
		
		<!-- #ifndef MP-ALIPAY -->
		<canvas :class="className" :canvas-id="canvasId" :width="higtCanvasSize" :height="higtCanvasSize" 
			:style="{ width: higtCanvasSize + 'px', height: higtCanvasSize + 'px' }" />
		<!-- #endif -->
	</view>
</template>
...
<script>
	...
	import { getStore, setStore, clearStore, circleImg, clacTextLen, downloadFile, pathToBase64, base64ToPath } from './utils/almost-utils.js'
	import { clearCacheFile } from './utils/almost-utils.js'
	export default {
		name: 'AlmostLottery',
		props: {
			// 设计稿的像素比基准值
			...
			lotteryBg: {
				type: String,
				default: require('./static/almost-lottery/almost-lottery__bg2x.png')
			},
			// 抽奖按钮背景图
			actionBg: {
				type: String,
				default: require('./static/almost-lottery/almost-lottery__action2x.png')
			},
			// 是否绘制奖品名称
			prizeNameDrawed: {
				type: Boolean,
				...
				if (this.imgDrawed && prizeItem.prizeImage && this.strDirection !== 'vertical') {
					...
					if (reg.test(prizeItem.prizeImage)) {
						...
						if (res.ok) {
							...
							this.handlePrizeImgSuc({ok: false,data: res.data,msg: res.msg})
						}
					} else {
						// #ifndef MP
					...
					let drawTimer = setTimeout(() => {
						...
						uni.canvasToTempFilePath({
							...
							success: (res) => {
								...
								this.handlePrizeImg({ok: true,data: res.tempFilePath,msg: '画布导出生成图片成功'})
							},
							fail: (err) => { this.handlePrizeImg({ok: false,data: err,msg: '画布导出生成图片失败'}) }
						}, this)
						// #endif
					}, 500)
				})
