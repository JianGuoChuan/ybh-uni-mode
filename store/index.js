...
try{
	...
	themeData = uni.getStorageSync('themeData');
}catch(e){
	
}

// 需要永久存储，且下次APP启动需要取出的，在state中的变量名
let saveStateKeys = ['vuex_userinfo', 'vuex_token','vuex_themeColor','vuex_versionCode','vuexBaseUrl'];
...
const saveLifeData = function(key, value){
	...
	if(saveStateKeys.indexOf(key) != -1) {
		...
		if(key == 'vuex_themeColor'){
			let themeTmp = uni.getStorageSync('themeData');
			themeTmp = themeTmp ? themeTmp : {};
			themeTmp[key] = value;
			uni.setStorageSync('themeData', themeTmp);
			return
		}
...
const store = new Vuex.Store({
	state: {
		...
		vuex_themeColor: themeData.vuex_themeColor ? themeData.vuex_themeColor : {},
